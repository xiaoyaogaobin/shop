...
@section('content')
    ...
    <div class="gome_container">
        <!--左-->
        <div class="prd_firstscreen_left" style="position: relative">

        @include('home.index.layouts.sild')
        <!---->
            <div class="preview-info" style="width: 381px;">
                <div class="left-btns">
        ...
        <!--中-->
        <div class="prd_firstscreen_center" style="position:absolute;right:440px">
            <div class="hgroup">
                <h1>{{$content['title']}}</h1>
            </div>
                            ...
                            @foreach($contents as $pecs)
                                <div class="prdmod">
                                    <a href="javascript:;"onclick="chooseSpec({{$pecs['id']}},this)"
                                       title="{{$pecs['spec']}}" spec="{{$pecs['id']}}"
                                    data-alt="{{$pecs['spec']}}">{{$pecs['spec']}}<i></i></a>
                                </div>
                            @endforeach
                        </div>
                    </div>
                    ...
                    function addCart(obj) {
                        ...
                        if($(obj).hasClass('bgclass')){
                            ...
                            $.ajax({
                                ...
                                data:{
                                    id:'{{$content['id']}}',
                                    spec:$('.prdRight_1 .prdmod').find('.select').attr('spec'),
                                    num:$('.quantity').val()
                                },
                                //规定php 返回类型
                                dataType:'json',
